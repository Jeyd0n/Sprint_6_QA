...
from utils.data import YaScooterOrderPageData as order_data
...
class TestYaScooterOrderPage:
    ...
    def test_order_page_first_name_input_incorrect_show_error_message(
        self, 
        driver
    ):
        ...
        ya_scooter_order_page.input_first_name('npc')
    ...
    def test_order_page_last_name_input_incorrect_show_error_message(
        self, 
        driver
    ):
        ya_scooter_order_page = YaScooterOrderPage(driver)
        ...
        ya_scooter_home_page.click_cookie_accept()
        ya_scooter_order_page.input_last_name('npc')
        ya_scooter_order_page.go_next()
    ...
    def test_order_page_address_input_incorrect_show_error_message(
        self, 
        driver
    ):
        ya_scooter_order_page = YaScooterOrderPage(driver)
        ...
        ya_scooter_home_page.click_cookie_accept()
        ya_scooter_order_page.input_address('npc')
        ya_scooter_order_page.go_next()
    ...
    def test_order_page_subway_input_empty_show_error_message(
        self, 
        driver
    ):
        ya_scooter_order_page = YaScooterOrderPage(driver)
    ...
    def test_order_page_telephone_number_input_incorrect_show_error_message(
        self, 
        driver
    ):
        ya_scooter_order_page = YaScooterOrderPage(driver)
        ...
        ya_scooter_home_page.click_cookie_accept()
        ya_scooter_order_page.input_telephone_number('npc')
        ya_scooter_order_page.go_next()
    ...
    @allure.description(
        """
        Проверка что при корректных заполненных данных на этапе "Для кого самокат"
        нажатии "Далее" происходит переход на следующий этап "Про аренду"
        """
    )
    def test_order_page_go_to_choose_scooter_user_data_correct_open_about_rent(
        self, 
        driver
    ):
        ya_scooter_order_page = YaScooterOrderPage(driver)
        ...
        ya_scooter_home_page.click_cookie_accept()
        ya_scooter_order_page.fill_user_data(order_data.data_sets['first_data'])
        ya_scooter_order_page.go_next()

        assert len(
            ya_scooter_order_page.find_elements(YaScooterOrderPageLocator.ORDER_BUTTON)
        ) > 0

    @allure.title('Заполнение данных на этапе "Про аренду" и оформление заказа')
    @allure.description(
        """
        Проверка что при корреткных заполненных данных на этапе "Про аренду"
    ...
    def test_order_page_about_rent_input_correct_data_and_order_show_order_number(
        self, 
        driver, 
        data_set
    ):
    ...
    def test_order_page_create_order_and_go_order_status(
        self, 
        driver, 
        data_set
    ):
